...
function parseReabankFile(filepath)
  reaper.ShowConsoleMsg("[PARSE] Opening: " .. filepath .. "\n")
  local file = io.open(filepath, "r")
  if not file then
    reaper.ShowConsoleMsg("[PARSE] FAILED to open file!\n")
    return nil
  end
  reaper.ShowConsoleMsg("[PARSE] File opened successfully\n")
...
function getReaticuateBank(track)
  ...
  if trackName then
    ...
    if reabankCache[lowerName] then
      return reabankCache[lowerName]
    end

    -- Second pass: Find best partial match (prefer longer matches)
    local bestMatch = nil
    local bestScore = 0
    ...
    for key, bank in pairs(reabankCache) do
      if type(key) == "string" and type(bank) == "table" and bank.name then
        ...
        if bankLower == lowerName then
          return bank
        end

        -- Calculate match score based on common characters
        local score = 0
        if bankLower:find(lowerName, 1, true) then
          -- Track name is contained in bank name
          score = #lowerName * 2
        elseif lowerName:find(bankLower, 1, true) then
          -- Bank name is contained in track name - prefer longer bank names
          score = #bankLower
        end
